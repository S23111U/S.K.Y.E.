...
# TELEMETRY
# =========================================================
def save_telemetry(user_input, final_output, guardrails, call_func_used, tool_name=None, tool_args=None):
    global TURN_INDEX
    event = {
...
register_tool("get_weather", lambda **kw: Get_Info()[1] + ": " + str(Get_Info()[2]) + "°C, " + Get_Info()[3])
register_tool("set_alarm", lambda **kw: threading.Thread(target=set_alarm, args=(kw.get("time"),), daemon=True) or f"Alarm set for {kw.get('time')}.")
register_tool("set_reminder", lambda **kw: threading.Thread(target=set_reminder, args=(kw.get("time"), kw.get("task")), daemon=True) or f"Reminder set: {kw.get('task')} at {kw.get('time')}.")
...
def get_skye_response(user_input: str) -> str:
    ...
    if tool_name:
        ...
        tool_reply_str = json.dumps(tool_reply, ensure_ascii=False) if isinstance(tool_reply, (dict, list)) else str(tool_reply)
        ...
        trunc_len = 1000
        LAST_TOOL_RESULT = tool_reply_str[:trunc_len] + "... [truncated due to length]" if len(tool_reply_str) > trunc_len else tool_reply_str
    else:
    ...
    save_telemetry(user_input, final_output, guardrails, bool(tool_name), tool_name, tool_args)
    
    return final_output

def rule_based_response(speech: str):
    s = speech.lower()
    if any(g in s for g in ["hi skye", "hello skye", "good morning skye", "good evening skye"]):
        return Greetings()
    if s.startswith("open "):
        target = s.split("open ", 1)[1].strip()
        sites = {"youtube": "https://youtube.com", "google": "https://google.com", "spotify": "https://open.spotify.com"}
        if target in sites:
